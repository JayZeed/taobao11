...
public class connDb {
    ...
    public static void startConn(){
        try{
            ...
            //连接数据库中间件
            try{
                con = DriverManager.getConnection("jdbc:MySQL://localhost:3306/dbtaobao","root","zjjip19951119");
    ...
    public static ArrayList index_5() throws SQLException{
        ...
        startConn();
        stmt = con.createStatement();
        rs = stmt.executeQuery("select age_range,count(*) num from user_log  group by age_range desc");
        while(rs.next()){
            String[] temp={rs.getString("age_range"),rs.getString("num")};
            list.add(temp);
        }
        endConn();
        return list;
